...
| **Zahlungsuebersicht als Arbeitsplatz (Tabelle in voller Breite)** | Alle Zahlungen werden seitenweise vollstaendig geladen und als Tabelle gezeigt: sortierbare Kopfzeilen (Datum, Betrag, Kategorie, Zuordnung), Monatsgruppen mit Zwischensumme, Filter nach Kategorie, Zuordnung und Zeitraum (mit Vorgaben), Suche ueber Namen, Verwendungszweck, IBAN, Betrag und Datum, Summe der Treffer, Tastaturnavigation (Pfeile, Enter, Bild auf/ab), Detailspalte rechts (ausblendbar), Vollbild; gefenstert (nur sichtbare Zeilen im DOM), Seiten parallel geladen. Logik in `utils/zahlungenAnsicht.ts` (getestet). | `src/components/controlboard/ZahlungenArbeitsplatz.tsx`, `ZahlungenTabelle.tsx`, `ZahlungInspector.tsx` | fertig |
