...
| **Vorschau- und Korrekturmaske vor der Uebernahme** | Vollbild-Pruefmaske, eine Zeile je Buchung; Sichten Offen / Unsicher / Geaendert / Zugeordnet, Suche, „Naechster Prueffall"; Konfidenz, Begruendung und Korrektur (Kategorie, Vertrag oder Objekt) in der Detailspalte rechts; Tastatur: Pfeile blaettern, Leertaste waehlt ab/an. Logik in `utils/zuordnungsvorschlaege.ts` (getestet). | `src/components/controlboard/PaymentAssignmentResultsModal.tsx` | fertig |
| **Manuelle Vertrags- oder Objektauswahl in der Pruefmaske** | Durchsuchbare Liste aller Vertraege (aktiv, gekuendigt, beendet — mit Status, Laufzeit ueber getVertragsende(), Gesamtmiete) beziehungsweise aller Objekte, direkt in der Detailspalte statt in einem zweiten Fenster; „Zuordnung entfernen". Eigener Query-Key `zuordnung-vertraege-auswahl`. | `src/components/controlboard/ZuordnungsAuswahl.tsx` | fertig |
| **Uebernahme in die Datenbank** | Legt je Zahlung eine Zeile an oder aktualisiert eine vorhandene, speichert auch abgewaehlte und Nichtmiete-Zahlungen; bestehende Buchungen der betroffenen Tage und Bankverbindungen der Vertraege werden einmal je Import vorab geladen, der Abgleich je Zeile laeuft in `utils/zahlungenUebernahme.ts` (getestet). Fehler werden je Zeile protokolliert. | `src/components/controlboard/PaymentManagement.tsx (handleApplyAssignments)` | teilweise |
...
| **Upload-Historie und Nachschau des letzten Imports** | Listet die letzten zehn Uploads mit Zeitraum, Anzahl und Status (teilweise fehlgeschlagen wird rot markiert); ein Klick auf den juengsten zeigt die damals gespeicherten Zahlungen. | `src/components/controlboard/PaymentManagement.tsx (uploadHistory) und LastUploadReviewModal.tsx` | teilweise |
...
| **Zahlungsuebersicht als Arbeitsplatz (Tabelle in voller Breite)** | Alle Zahlungen werden seitenweise vollstaendig geladen und als Tabelle gezeigt: sortierbare Kopfzeilen (Datum, Betrag, Kategorie, Zuordnung), Monatsgruppen mit Zwischensumme (standardmaessig eingeklappt, Klick oeffnet, Knopf fuer alle; bei Textsuche offen), Filter nach Kategorie, Zuordnung und Zeitraum (mit Vorgaben), Suche ueber Namen, Verwendungszweck, IBAN, Betrag und Datum, Summe der Treffer, Tastaturnavigation (Pfeile, Enter, Bild auf/ab), Detailspalte rechts (ausblendbar), Vollbild; gefenstert (nur sichtbare Zeilen im DOM), Seiten parallel geladen. Logik in `utils/zahlungenAnsicht.ts` (getestet). | `src/components/controlboard/ZahlungenArbeitsplatz.tsx`, `ZahlungenTabelle.tsx`, `ZahlungInspector.tsx` | fertig |
...
| **Nebenkosten-Zuordnung: Ausgaben einem Objekt zuweisen** | Reiter „Nebenkosten" als Arbeitsplatz wie „Alle Zahlungen": gefensterte Tabelle mit Monatsgruppen, Sichten „Ohne Objekt / Zugeordnet / Ausgeblendet" mit Zaehlwerten, Kategoriefilter, Suche, Zeitraum; rechts die Detailspalte mit KI-Vorschlag (Kostenart, Konfidenz, Begruendung, „Vorschlag uebernehmen"), durchsuchbarer Objektliste mit Zaehlwerten, „Zum Objekt", Kategorie-Aktionen (als Nichtmiete/Nebenkosten, ausblenden, wieder einblenden). Knopf „KI-Vorschlaege" ruft classify-nebenkosten. Beide Abfragen mit range(). Logik in `utils/nebenkostenZuordnung.ts` (getestet). Ziehen und Ablegen entfaellt. | `src/components/controlboard/NebenkostenArbeitsplatz.tsx`, `NebenkostenInspector.tsx` | fertig |
