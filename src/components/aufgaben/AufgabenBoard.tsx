...
export const AufgabenBoard = ({ onBack, aufgabeOeffnen, onAufgabeGeoeffnet }: AufgabenBoardProps) => {
  ...
  // Eine von außen übergebene Aufgabe (etwa nach dem Melden) wird einmalig übernommen
  ...
  const gefiltert = useMemo(() => {
    ...
    const gefilterteListe = aufgaben.filter((a) => {
      ...
      if (sicht === "meine") {
        const meine = a.verantwortlich_id === ichSelbst?.id || a.melder_id === ichSelbst?.id;
  ...
  const meineAnzahl = aufgaben.filter(
    (a) =>
      a.status !== "fertig" && a.verantwortlich_id === ichSelbst?.id,
  ).length;
